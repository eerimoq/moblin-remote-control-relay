...
function updateStatsBridges(body, stats) {
  ...
  appendRow(
    ...
    "Bridges",
    "Streamers connected",
    stats.bridges.streamersConnected
  );
}
...
function updateStatsTrafficBridgesToStreamers(body, stats) {
  appendRow(
    body,
    "Traffic / Bridges to streamers",
    "Total bytes",
    bytesToString(stats.traffic.bridgesToStreamers.totalBytes)
  );
  appendRow(
    body,
    "Traffic / Bridges to streamers",
    "Current bitrate",
    bitrateToString(stats.traffic.bridgesToStreamers.currentBitrate)
  );
}
...
function updateStatsTrafficStreamersToBridges(body, stats) {
  appendRow(
    body,
    "Traffic / Streamers to bridges",
    "Total bytes",
    bytesToString(stats.traffic.streamersToBridges.totalBytes)
  );
  appendRow(
    body,
    "Traffic / Streamers to bridges",
    "Current bitrate",
    bitrateToString(stats.traffic.streamersToBridges.currentBitrate)
  );
}
